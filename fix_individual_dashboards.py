...
def fix_individual_student_dashboards():
    """Fix JavaScript template syntax in individual dashboard files"""
    
    # Connect to database
    conn = sqlite3.connect('library_main.db')
    cursor = conn.cursor()
    
    print('🔧 FIXING INDIVIDUAL STUDENT DASHBOARDS')
    print('=' * 60)
    ...
    for i, (student_id, name, roll_number, branch) in enumerate(students, 1):
        print(f'\n📝 Fixing dashboard for Student {i}: {name} ({roll_number}) - {branch}')
        ...
        unpaid_fines = len([f for f in fines if f[4] == 'Unpaid'])
        ...
        # Create individual dashboard HTML with proper JavaScript escaping
...
<body>
    ...
    conn.close()
    print(f'\n🎉 INDIVIDUAL DASHBOARDS FIXED: {len(students)} files')

if __name__ == "__main__":
    fix_individual_student_dashboards()
